...
1) **Уникальность имён (переменных):** левая часть присваивания не должна дублироваться в рамках области видимости.	
...
2) **Уникальность имён (параметров):** параметры лямбды не должны дублироваться.	
...
в теле лямбды можно использовать только те переменные, которые были переданы в параметрах.	

(a, b) -> a + c;	

Семантическая ошибка: переменная 'c' не объявлена

4) **Совместимость типов / валидность операндов:** бинарные операции требуют наличия левого и правого операндов.

x -> x + ;	

Семантическая ошибка: неверные операнды для оператора +

5) **Допустимые значения:** проверка на выход числовых констант за пределы типов 
(например, переполнение Integer).

x -> x + 9999999999999999;

Семантическая ошибка: число 9999999999999999 выходит за допустимые пределы
